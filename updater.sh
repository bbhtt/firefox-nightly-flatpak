...
sed '155!d' $MANIFEST_PATH|sed -n '{s|.*/firefox-\(.*\)\.tar.bz2|\1|p;q;}' > oldrelease;
...
newrelease=$(cat newrelease) && oldrelease=$(cat oldrelease) && sed -i "155s/$oldrelease/$newrelease/g" $MANIFEST_PATH;

# Extract the filename again in case of an update
sed '155!d' $MANIFEST_PATH|sed -n '{s|.*/firefox-\(.*\)\.tar.bz2|\1|p;q;}' > updrelease;
...
sed '156!d' $MANIFEST_PATH|cut -d: -f2|sed 's/^[ \t]*//;s/[ \t]*$//' > shaold;

# Replace the checksum
shanew=$(cat shanew) && shaold=$(cat shaold) && sed -i "156s/$shaold/$shanew/g" $MANIFEST_PATH;
